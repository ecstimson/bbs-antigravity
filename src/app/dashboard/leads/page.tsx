...
import { MOCK_LEADS } from '@/lib/dashboard/constants';
            ...
            <CardContent className="p-0">
               ...
               <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-4 justify-between items-center">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                     <button className="flex items-center gap-2 px-3 py-2 bg-surface border border-border rounded-lg text-sm hover:bg-border transition-colors text-muted hover:text-white">
                        <Filter size={16} />
                        Filter
                     </button>
                     <button className="flex items-center gap-2 px-3 py-2 bg-surface border border-border rounded-lg text-sm hover:bg-border transition-colors text-muted hover:text-white">
                     ...
                     <tbody className="divide-y divide-border">
                        {filteredLeads.map(lead => (
                           <tr key={lead.id} className="hover:bg-surface/30 transition-colors group">
                              ...
                              <td className="px-6 py-4 font-medium text-gray-300">{lead.value}</td>
                              <td className="px-6 py-4 text-muted">{lead.source}</td>
                              <td className="px-6 py-4 text-muted">{lead.date}</td>
                              <td className="px-6 py-4 text-right">
                                 <button className="p-2 text-muted hover:text-white hover:bg-border rounded-lg transition-colors">
